...
public abstract class SimpleWidgetCoreX<T extends SimpleWidgetCoreX<T>> extends RenderWidgetCore<T> {
	...
	protected SimpleWidgetCoreX(int x, int y, int w, int h, Component pMessage) {
		this(x, y, w, h, 2, pMessage);
	}

	protected SimpleWidgetCoreX(int x, int y, int w, int h, int r, Component pMessage) {
		super(x, y, w, h, pMessage);
		setRadius(r);
	}

	protected SimpleWidgetCoreX(int x, int y, int w, int h, int radius, int borderUsualColor, int borderHoverColor, int textUsualColor, int textHoverColor, Component pMessage) {
		super(x, y, w, h, pMessage);
	...
	protected SimpleWidgetCoreX(int x, int y, int w, int h, int radius, int borderUsualColor, int borderHoverColor, int textUsualColor, int textHoverColor, int backgroundUsualColor, int backgroundHoverColor, Component pMessage) {
		super(x, y, w, h, pMessage);
	 ...
	 * 绘制边框发光。
	 *
	 * 使用多层半透明边框模拟软发光，不依赖额外纹理或 Shader。
	 * 越靠近边框的层透明度越高，越远透明度越低。
	 */
	protected void renderGlow(GuiGraphicsExtractor guiGraphics, int glowColor, boolean inner) {
		...
		for (int distance = range; distance >= 1; distance--) {
			...
			/*
			 * 二次衰减：
			 * distance=1 时接近 intensity；
			 * 越远衰减越明显，形成更自然的软边缘。
			 */
			final float normalized = 1.0f - ((float) (distance - 1) / (float) range);
			final float alphaFactor = normalized * normalized * intensity;
