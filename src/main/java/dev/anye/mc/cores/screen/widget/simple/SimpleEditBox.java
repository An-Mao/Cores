...
public class SimpleEditBox extends SimpleWidgetCore<SimpleEditBox>{
    ...
    private void deleteText(int count, boolean p_446878_) {
        if (p_446878_) {
    ...
    @Override
    public boolean keyPressed(KeyEvent p_445707_) {
        return keyPressed(p_445707_.key(),p_445707_.scancode(),p_445707_.modifiers(),p_445707_) || super.keyPressed(p_445707_);
    }

    public boolean keyPressed(int pKeyCode, int pScanCode, int pModifiers,KeyEvent keyEvent) {
    ...
    public void onClick(MouseButtonEvent p_446284_, boolean p_434599_) {
        int i = Mth.floor(p_446284_.x()) - this.getX();
        i -= getRadius() * 2 ;
        String s = this.font.plainSubstrByWidth(this.value.substring(this.displayPos), this.getInnerWidth());
        this.moveCursorTo(this.font.plainSubstrByWidth(s, i).length() + this.displayPos, p_446284_.hasShiftDown());
    }
    ...
    protected void renderContent(GuiGraphicsExtractor guiGraphics, int mouseX, int mouseY, float partialTick) {
        ...
        int startX = this.getContentX() + 2;
        int startY = this.getContentY() + getContentH() / 2 - font.lineHeight / 2;
        int l = startX;
        ...
        if (!showText.isEmpty() && flag && pos < showText.length()) {
            guiGraphics.text(this.font, this.formatter.apply(showText.substring(pos), this.cursorPos), l, startY, textColor,false);
        }

        if (this.hint != null && showText.isEmpty() && !this.isFocused()) {
            guiGraphics.text(this.font, this.hint, l, startY, textColor,false);
        }

        if (!flag2 && this.suggestion != null) {
            guiGraphics.text(this.font, this.suggestion, j1 - 1, startY, -8355712,false);
        }

        if (showFocus) {
            guiGraphics.fill(RenderPipelines.GUI, j1, startY - 1, j1 + 1, startY + 1 + 9, -3092272);
        }
    ...
    private void renderHighlight(GuiGraphicsExtractor pGuiGraphics, int pMinX, int pMinY, int pMaxX, int pMaxY) {
        if (pMinX < pMaxX) {
            int i = pMinX;
            pMinX = pMaxX;
            pMaxX = i;
        ...
        pGuiGraphics.fill(RenderPipelines.GUI_TEXT_HIGHLIGHT, pMinX, pMinY, pMaxX, pMaxY, this.backgroundSelectColor);
    }
