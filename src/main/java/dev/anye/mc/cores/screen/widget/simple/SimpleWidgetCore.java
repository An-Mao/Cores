...
public abstract class SimpleWidgetCore<T extends SimpleWidgetCore<T>> extends RenderWidgetCore<T> {
    ...
    protected void drawRoundedRect(GuiGraphics guiGraphics, int x, int y, int width, int height, int radius, int borderColor, int fillColor) {
        ...
        guiGraphics.drawSpecial(multiBufferSource -> {
            VertexConsumer v = multiBufferSource.getBuffer(RenderType.gui());
            addVertex(v,matrix, x + radius, y + radius, fillColor);
            addVertex(v,matrix, x + width - radius, y + radius, fillColor);
            addVertex(v,matrix, x + width - radius, y + height - radius, fillColor);
            addVertex(v,matrix, x + radius, y + height - radius, fillColor);
        });
    ...
    protected void addVertex(VertexConsumer buffer, Matrix4f matrix, int x, int y, int color) {
        buffer.addVertex(matrix,x, y, 0).setColor((color >> 16) & 0xFF, (color >> 8) & 0xFF, color & 0xFF, (color >> 24) & 0xFF);//.endVertex();
    }
    @Override
