...
public class CWidgetEditBox extends CWidgetBaseCore<CWidgetEditBox>{
    ...
    public CWidgetEditBox(_BoundingBox boundingBox, BorderStyle borderStyle, Component pMessage) {
        this(boundingBox, borderStyle, null,pMessage);
    }
    public CWidgetEditBox(_BoundingBox boundingBox, BorderStyle borderStyle,CWidgetEditBox cWidgetEditBox ,Component pMessage) {
        super(boundingBox, borderStyle, pMessage);
        if (cWidgetEditBox != null) this.setValue(cWidgetEditBox.value);
    }
    ...
    public void onClick(double pMouseX, double pMouseY,int button) {
        int i = Mth.floor(pMouseX) - contentBox.getX();
        String s = this.font.plainSubstrByWidth(this.value.substring(this.displayPos), this.contentBox.getW());
    ...
    protected void renderContent(GuiGraphics guiGraphics, _BoundingBox boundingBox, int mouseX, int mouseY, float partialTick) {
        ...
        String showText = this.font.plainSubstrByWidth(this.value.substring(this.displayPos), boundingBox.getW());


        boolean flag = pos >= 0 && pos <= showText.length();
        boolean showFocus = this.isFocused() && (Util.getMillis() - this.focusedTime) / 300L % 2L == 0L && flag;
        int startX = boundingBox.getX();
        int startY = boundingBox.getY() + (boundingBox.getH() >> 1) - font.lineHeight / 2;
        int l = startX;
        int i1 = Mth.clamp(this.highlightPos - this.displayPos, 0, showText.length());
        if (!showText.isEmpty()) {
            String s1 = flag ? showText.substring(0, pos) : showText;
            l = guiGraphics.drawString(this.font, this.formatter.apply(s1, this.displayPos), startX, startY, textColor,false);
        }

        boolean flag2 = this.cursorPos < this.value.length() || this.value.length() >= this.getMaxLength();
        int j1 = l;
        if (!flag) {
            j1 = pos > 0 ? startX + this.width : startX;
        } else if (flag2) {
            j1 = l - 1;
            l--;
        }
        ...
        if (!showText.isEmpty() && flag && pos < showText.length()) {
            guiGraphics.drawString(this.font, this.formatter.apply(showText.substring(pos), this.cursorPos), l, startY, textColor,false);
        }

        if (this.hint != null && showText.isEmpty() && !this.isFocused()) {
            guiGraphics.drawString(this.font, this.hint, l, startY, textColor,false);
        }

        if (!flag2 && this.suggestion != null) {
            guiGraphics.drawString(this.font, this.suggestion, j1 - 1, startY, -8355712,false);
        }

        if (showFocus) {
            guiGraphics.fill(RenderType.guiOverlay(), j1, startY - 1, j1 + 1, startY + 1 + 9, -3092272);
        }

        if (i1 != pos) {
            int k1 = startX + this.font.width(showText.substring(0, i1));
            this.renderHighlight(guiGraphics, j1, startY - 1, k1 - 1, startY + 1 + 9);
        }
    }
    ...
    private void renderHighlight(GuiGraphics pGuiGraphics, int pMinX, int pMinY, int pMaxX, int pMaxY) {
        ...
        pGuiGraphics.fill(RenderType.guiTextHighlight(), pMinX, pMinY, pMaxX, pMaxY, this.backgroundSelectColor);
    }
