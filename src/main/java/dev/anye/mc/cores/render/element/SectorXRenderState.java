...
public record SectorXRenderState(
    ...
    public void buildVertices(@NotNull VertexConsumer vertexConsumer, float z) {
        ...
        for (int i = 0; i < segments; i++) {
            ...
            vertexConsumer.addVertexWith2DPose(pose, x1_inner, y1_inner, z).setColor(color);
            vertexConsumer.addVertexWith2DPose(pose, x1_outer, y1_outer, z).setColor(color);
            vertexConsumer.addVertexWith2DPose(pose, x2_outer, y2_outer, z).setColor(color);
            vertexConsumer.addVertexWith2DPose(pose, x2_inner, y2_inner, z).setColor(color);
        }
        pose.popMatrix();
    }
}
