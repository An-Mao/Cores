...
public record SectorRenderState(
    ...
    public SectorRenderState(Matrix3x2fStack pose,int x,int y,int outerRadius, double startArc, double endArc, int color,
                             @Nullable ScreenRectangle scissorArea,
    ...
    public void buildVertices(VertexConsumer vertexConsumer, float p_418216_) {
        ...
        vertexConsumer.addVertexWith2DPose(pose, 0, 0, p_418216_).setColor(color);
        ...
        for (int i = 0; i <= segments; i++) {
            ...
            vertexConsumer.addVertexWith2DPose(pose, x, y, p_418216_).setColor(color);
        }
        pose.popMatrix();
    }
}
